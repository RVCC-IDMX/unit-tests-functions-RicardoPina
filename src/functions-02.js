...
const store = {
  ...
  getExpensiveItems(minPrice) {
    const expensiveItems = inventory.filter((items) => items.price >= minPrice);
  ...
  /**
       ...
       */
  getStoreItems() {
    return inventory.map((itemObj) => itemObj.name);
  },
  /**
  ...
  isItemInStore(itemName) {
    const objFound = inventory.some((itemObj) => itemObj.name === itemName);
